...
library(infercnv)    # estimate CNVs
...
primary <- FindClusters(primary, 
                        ...
                        verbose = FALSE)
DimPlot(primary)
FeaturePlot(primary, features = "CD34")
ElbowPlot(primary)
```
...
[`SingleR`](https://www.nature.com/articles/s41590-018-0276-y) uses correlation to compare scRNA-seq data to a labeled reference dataset. We'll use the annotated cells from [Baron *et al* (2016)](https://pubmed.ncbi.nlm.nih.gov/27667365/) in order to give broad cell types to our clusters. Note: these should not be considered ground truth, but can help guide our manual annotations. 
...
primary[["SingleR"]] <- case_when(primary[["SingleR"]] == "acinar" ~ "Acinar", 
                                  ...
                                  primary[["SingleR"]] == "ductal" ~ "Epithelial", 
...
meta[["SingleR"]] <- case_when(meta[["SingleR"]] == "epsilon" ~ "Epsilon", 
                               ...
                               meta[["SingleR"]] == "ductal" ~ "Epithelial", 
...
Let's check out the results! I really doubt the Epsilon label for the metastatic samples, as Epsilon cells make up around 1% of all pancreatic islet cells. The T cells and Macrophage labels match up with the author's annotations. I would guess that the rest of the cells are split between normal ductal cells and PDAC cells. 

As for the primary tumor cells, we see a Macrophage cluster and an Activated Stellate (read: CAF) cluster which matches the authors' annotations. The acinar cells are most likely mislabeled EMT and Epithelial cells, which we'll investigate furthers. Finally, the Mast cells might be correctly labeled, or they might be another immune celltype. 

authors - metastatic has tumor-infiltrating leukocyte, tumor-associated macro, and epithelial tumor cells
        - primary has CAFs, DC, epithelial tumor, endothelial, TAMs, TILs, and EMTs

```{r}
(p7 / p6) + plot_annotation(title = "SingleR Cell Type Annotations", 
...
We'll use [Xianlu Peng's `DECODER` method](https://pubmed.ncbi.nlm.nih.gov/31628300/) in order to perform a pancreas-specific NMF version of pathway analysis, in which each cell is assigned compartment weights for pathways such as Immune, Classical PDAC, etc. 
...
p8 <- ggarrange(plotlist = plots, ncol = 3, nrow = 2) %>% 
      annotate_figure(bottom = "UMAP 1", left = "UMAP 2", top = "DECODER Pathway Analysis (Primary Tumor)")
p8
```
...
Despite some missing genes, we see 

```{r}
p9 <- ggarrange(plotlist = plots, ncol = 3, nrow = 2) %>% 
      annotate_figure(bottom = "UMAP 1", left = "UMAP 2", top = "DECODER Pathway Analysis (Metastatic Tumor)")
p9
```
